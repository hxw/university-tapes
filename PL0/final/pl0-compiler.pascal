  ...
  BEGIN IF CC=LL THEN
       ...
       CC:=0; WRITE(CX:5,' ');
        WHILE NOT EOLN(IP1) DO
         BEGIN READ(IP1,CH); WRITE(CH);
              IF ORD(CH)<127 THEN BEGIN LINE[LL]:=CH; LL:=LL+1 END
         END;
       WRITELN; READLN(IP1); LINE[LL]:=' '
     END;
       CC:=CC+1; CH:=LINE[CC]
  END(*GETCH*);
...
VAR  I:0..CXMAX;
  ...
  PROCEDURE OPTIMISE(FINISH:FTYP);
  VAR  I:FTYP;
       JMPORRET:BOOLEAN;
  BEGIN
       JMPORRET:=FALSE;
       ...
       WITH CODE[I] DO
       BEGIN
            IF JMPORRET AND NOT LABELED AND (F=JMP) THEN
            BEGIN
                 F:=OPR;  L:=0;  A:=99
            END ELSE
            JMPORRET:=((F=JMP) OR ((F=OPR) AND (A=0) ));
       END
  END;
  ...
  BEGIN
       WRITELN;
       DEFINEINPUT(7,U64(' PL0.LIB'));
       WHILE NOT EOF(IP7) DO
       BEGIN
            WHILE NOT EOLN(IP7) DO
            BEGIN
                 READ(IP7,CH); WRITE(CH)
            END;
            READLN(IP7); WRITELN
       END
  END;
...
BEGIN (*Z80*)
     ...
     BEGIN
          ...
          OPR:CASE A OF
              ...
              50:BEGIN (*READ*)
                      HL:=FULL;
                      WRITELN('    CALL READNUM')
                 END;
              51:BEGIN (*WRITE*)
                      HL:=EMPTY;
                      WRITELN('    CALL WRITENUM')
                 END;
              99:(*NO-OPERATION*)
              END (*OPR CASE*);
          END (*F CASE*)
...
BEGIN (*MAIN PROGRAM*)
     ...
     DEFINEINPUT(1,INNAME);
     WRITE(OUTPUT,'LISTING FILE NAME');
     DEFINEOUTPUT(1,INNAME,0,1000,1);
     WRITELN(OUTPUT,'Z80 ASSEMBLY CODE FILE NAME');
     DEFINEOUTPUT(2,INNAME,0,1000,1);
     WRITELN(OUTPUT,'P-CODE LISTING ? (YES/NO)');
     LIST:=INNAME=U64('     YES');
     SELECTOUTPUT(1);
     ...
     SELECTOUTPUT(0);
     IF ERR<>0 THEN WRITELN('ERRORS IN PL0 PROGRAM') ELSE Z80(CX-1);
     BREAKOUTPUT(1);
     BREAKOUTPUT(2);
END.
